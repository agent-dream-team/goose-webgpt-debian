...
export { insertPlainTextIntoComposer } from "../../chatgpt-composer";
import { setChatGptThinkMode } from "../../chatgpt-think-mode";
export { setChatGptThinkMode } from "../../chatgpt-think-mode";
import { chatGptNewTurnIdentity, chatGptReboundTurnIdentity } from "../../chatgpt-turn-identity";
export { chatGptNewTurnIdentity, chatGptReboundTurnIdentity } from "../../chatgpt-turn-identity";
import {
...
} from "../../chatgpt-turn-state";
import { detectChatGptTerminalError } from "../../chatgpt-terminal-state";
export {
  CHATGPT_COMPLETION_ACTION_GRACE_MS,
  CHATGPT_COMPLETION_SETTLE_MS,
  ChatGptCompletionTracker,
...
export async function throwIfChatGptTerminalErrorAlert(scope: ChatGptTextScope): Promise<void> {
  const terminal = await detectChatGptTerminalError(scope);
  if (!terminal) return;
  throw new ChatGptWebAdapterError(
    terminal === "regenerate-error"
...
export class ChatGptBrowserWorker {
  ...
  private async attachedPromptText(page: Page, abortSignal?: AbortSignal): Promise<string> {
    ...
    return composer.evaluate(readChatGptComposerPlainText, undefined, { timeout: 20_000, signal: abortSignal });
  }

...
import { startPersistentRebuildService } from "./rebuild-runtime-service";
...
async function main(): Promise<void> {
  ...
  if (command === "help") stdout.write(HELP);
    ...
    assertNoArgs(args);
    const config = loadConfig();
    if (config.runtimeKind === "persistent-rebuild") {
      const service = startPersistentRebuildService(config);
      const stop = () => { void service.stop(); };
      process.once("SIGINT", stop);
      process.once("SIGTERM", stop);
      stdout.write(`goose-chatgpt-web rebuild ${VERSION} listening on ${service.runtime.origin}/v1 (${config.mode})\n`);

...
> **CURRENT GOOSE V1:** Goose ChatGPT Web has a qualified non-reboot V1 on DreamBook. The canonical DreamBook repository is `agent-dream-team/goose-webgpt-debian`; `luke-m-selway/goose-chatgpt-web` is a separate future cross-platform/macOS adaptation target, not a DreamBook deployment remote. The exact deployed code checkpoint is `2ac2f561a674482d8eccf0e0cc690edc095b7806`; the stable installed appliance uses the package-owned launcher/autostart path and persistent canonical ChatGPT conversations. Actual DreamBook reboot reconstruction remains a deferred proof, not a V1 blocker. The remainder of this README is inherited Codex-oriented upstream documentation and is **reference material, not an operational setup guide**. Current authority is `AGENTS.md` plus `docs/persistent-chat-rebuild-plan.md`.

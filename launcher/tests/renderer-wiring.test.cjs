...
test("DEV launcher exposes its profile and supervises only its Full-mode MCP runtime", () => {
  ...
  assert.match(
    electronMain,
    /packaged:\s*updatesEnabledForAppliance\(\{[\s\S]*?packaged:\s*app\.isPackaged,[\s\S]*?devProfile:\s*IS_DEV_PROFILE,[\s\S]*?applianceKind:\s*applianceKind\(\),?[\s\S]*?\}\)/,
  );
...
test("persistent rebuild identifies itself and suppresses inherited Codex setup workflow", () => {
  assert.match(electronMain, /GOOSE_CHATGPT_WEB_APPLIANCE/);
  assert.match(electronMain, /applianceKind:\s*applianceKind\(\)/);
  ...
  assert.match(appSource, /!rebuildAppliance \? <SidebarGroup label=\{copy\.configuration\}>/);
  assert.match(appSource, /!rebuildAppliance && surface === "setup"/);
  assert.match(appSource, /!rebuildAppliance && surface === "mcp"/);
  assert.match(appSource, /GOOSE REBUILD/);
  assert.match(appSource, /LEGACY CODEX/);
});

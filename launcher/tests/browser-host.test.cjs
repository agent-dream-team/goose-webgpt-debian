...
test("automatic turn view presentation ignores a vanished WebContents", () => {
  const calls = [];
  const tab = {
    interactionMode: "automatic", status: "running", rendererReady: true,
    deviceEmulationViewport: { width: 1200, height: 800 }, deviceEmulationDirty: false,
    view: { webContents: undefined, setBounds: () => calls.push("bounds"), setVisible: () => calls.push("visible") },
  };
  const fixture = Object.assign(Object.create(BrowserHost.prototype), {
...
test("a destroyed running disposable surface is reaped before exact-trace reconstruction", async () => {
  const conversationKey = "z".repeat(64);
  const destroyed = {
    id: "destroyed-running", surfaceId: "surface-old", traceId: "trace_next", helperPid: 111,
    status: "running", interactionMode: "automatic", conversationKey, connectorIdentity: "Codex Native2",
    connectorBound: true,
    view: { webContents: undefined },
  };
  const created = { id: "replacement", surfaceId: "surface-replacement" };
  const events = [];
  const fixture = Object.assign(Object.create(BrowserHost.prototype), {
    manualOperation: null,
    ...
    closedTurnOwners: new Map(),
    userCancelledTurnOwners: new Map(),
    selectedTabId: destroyed.id,
    window: { contentView: { removeChildView: () => events.push("removed") } },
    createTurnTab: (...args) => {
  ...
  const lease = await BrowserHost.prototype.beginTurn.call(
    fixture, "trace_next", false, 222, conversationKey, "Codex Native2", true,
  );
  assert.deepEqual(lease, {
    surfaceId: "surface-replacement", tabId: "replacement", reused: false,
    connectorBound: false, surfaceRecreated: true,
  });
  assert.equal(fixture.turnTabs.has(destroyed.id), false);
  assert.ok(events.includes("browser.destroyed_turn_surface_reaped"));
  assert.ok(events.includes("browser.retained_surface_recreated"));
});

test("ending a turn whose WebContents vanished releases the disposable tab without throwing", async () => {
  const tab = {
    id: "missing-contents", surfaceId: "surface-missing", traceId: "trace_missing", helperPid: 444,
    status: "running", interactionMode: "automatic", conversationKey: "m".repeat(64),
    connectorIdentity: "Codex Native2", connectorBound: true, loading: true,
    view: { webContents: undefined },
  };
  const events = [];
  const fixture = Object.assign(Object.create(BrowserHost.prototype), {
    ...
    syncPowerSaveBlocker() {}, syncViewVisibility() {}, hide: () => events.push("hidden"),
    snapshot: () => ({ tabs: [] }), publishState() {}, writeDescriptor: () => events.push("descriptor"),
    logger: { info: event => events.push(event) },
  });
  const result = await BrowserHost.prototype.endTurn.call(
    fixture, tab.traceId, tab.helperPid, "failed", false, "observer lost", true, true,
  );
...
test("a completed turn cannot retain connector authority after its WebContents vanished", async () => {
  const tab = {
    id: "missing-completed", surfaceId: "surface-missing-completed", traceId: "trace_missing_completed", helperPid: 445,
    status: "running", interactionMode: "automatic", conversationKey: "n".repeat(64),
    connectorIdentity: "Codex Native2", connectorBound: true, loading: true,
    view: { webContents: undefined },
  };
  const events = [];
  const fixture = Object.assign(Object.create(BrowserHost.prototype), {
    ...
    selectedTabId: tab.id, window: { contentView: { removeChildView: () => events.push("removed") } },
    view: { webContents: { getURL: () => IDLE_BROWSER_URL } },
    syncPowerSaveBlocker() {}, syncViewVisibility() {}, hide: () => events.push("hidden"),
    snapshot: () => ({ tabs: [] }), publishState() {}, writeDescriptor: () => events.push("descriptor"),
    logger: { info: event => events.push(event) },
  ...
  const result = await BrowserHost.prototype.endTurn.call(
    fixture, tab.traceId, tab.helperPid, "completed", true, undefined, true, true,
  );
  assert.deepEqual(result, { cancelledByUser: false });
  assert.equal(fixture.turnTabs.has(tab.id), false);
  assert.equal(tab.connectorBound, true);
  assert.ok(events.includes("browser.tab_completed"));
  assert.ok(events.includes("browser.tab_released"));
  assert.ok(!events.includes("browser.tab_retained"));
});

test("a required durable conversation recreates a disposable browser surface when no retained tab survives", async () => {
  ...
  assert.equal(fixture.selectedTabId, "replacement");
  assert.deepEqual(events, ["visible", "published", "browser.retained_surface_recreated", "descriptor"]);
});

test("required durable-conversation recovery refuses conflicting retained connector metadata", async () => {
  ...
  const fixture = Object.assign(Object.create(BrowserHost.prototype), {
    ...
    turnTabs: new Map([[retained.id, retained]]),
    userCancelledTurnOwners: new Map(),
    createTurnTab: () => { created = true; throw new Error("must not allocate over conflicting metadata"); },
  });
  ...
  await assert.rejects(
    ...
    /conflicting connector metadata/,
  );

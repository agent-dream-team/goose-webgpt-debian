...
test("persistent rebuild disables the inherited upstream launcher updater", () => {
  assert.equal(updatesEnabledForAppliance({
    packaged: true, devProfile: false, applianceKind: "persistent-rebuild",
  }), false);
  assert.equal(updatesEnabledForAppliance({
    packaged: true, devProfile: false, applianceKind: "legacy-codex",
  }), true);
  assert.equal(updatesEnabledForAppliance({
    packaged: false, devProfile: false, applianceKind: "legacy-codex",
  }), false);
});

...
test("Linux runtime bundle owns the DreamBook account fence and headless launcher", () => {
  const builder = fs.readFileSync(path.join(repositoryRoot, "scripts", "build-runtime-bundle.ts"), "utf8");
  assert.match(builder, /dreambook-account-browser-lease\.ts/);
  assert.match(builder, /dreambook-account-browser-lease\.js/);
  assert.match(builder, /dreambook-headless-xvfb-runner\.ts/);
  assert.match(builder, /dreambook-headless-xvfb-runner\.js/);
  assert.match(builder, /target:\s*"bun"/);
});

test("Linux runtime bundle owns the persistent rebuild tunnel authorization wrapper", () => {
  const builder = fs.readFileSync(path.join(repositoryRoot, "scripts", "build-runtime-bundle.ts"), "utf8");
  assert.match(builder, /dreambook-rebuild-tunnel-client-auth-wrapper\.sh/);
  assert.match(builder, /process\.platform === "linux"/);
  assert.match(builder, /chmodSync\(rebuildTunnelWrapper, 0o755\)/);
});

...
const { assertLegacyCodexAction, resolveApplianceKind } = require("./appliance-kind.cjs");
...
const { createUpdateController, updatesEnabledForAppliance } = require("./update.cjs");
const {
  createStateStore,
...
async function restoreCodexRouteAfterRuntimeFailure({ logger, stateStore }) {
  if (runtimeHost?.runtimeConfigSnapshot().config?.runtimeKind === "persistent-rebuild") {
...
function applianceKind() {
  return resolveApplianceKind({
    declared: process.env.GOOSE_CHATGPT_WEB_APPLIANCE,
    runtimeSnapshot: runtimeHost?.runtimeConfigSnapshot(),
  });
}

function requireLegacyCodexAction(action) {
  assertLegacyCodexAction(applianceKind(), action);
}
...
function registerIpc({ logger, stateStore }) {
  ...
  handle("launcher:complete-onboarding", (_event, language, rawInteractionMode) => {
    const current = stateStore.read();
    const persistentRebuild = applianceKind() === "persistent-rebuild";
    if (!persistentRebuild && (!current.githubOpened || !current.xOpened)) {
      throw new Error("Open the GitHub and X pages before continuing");
    }
    if (current.autoStart) setAutostart(app, true);
    const next = stateStore.update({
      language: validateLanguage(language),
      browserInteractionMode: persistentRebuild ? "automatic" : validateBrowserInteractionMode(rawInteractionMode),
  ...
  handle("launcher:browser-smoke", async () => {
    requireLegacyCodexAction("Browser smoke setup");
  ...
  handle("launcher:mcp-verify", async (event) => {
    requireLegacyCodexAction("Codex MCP verification");
  ...
  handle("launcher:uninstall-integration", async () => {
    requireLegacyCodexAction("Codex integration removal");
  ...
  handle("launcher:setup-core", async () => {
    requireLegacyCodexAction("Codex core setup");
  ...
  handle("launcher:setup-mcp", async (_event, input) => {
    requireLegacyCodexAction("Codex MCP setup");
  ...
  handle("launcher:zero-risk-pro", async (_event, enabled) => {
    requireLegacyCodexAction("Zero Risk setup");
  ...
  handle("launcher:browser-interaction-mode", async (_event, rawMode) => {
    if (applianceKind() === "persistent-rebuild") {
      if (rawMode !== "automatic") {
        throw new Error("Persistent Goose rebuild uses Automatic browser interaction; inherited Zero Risk/manual mode is disabled");
      }
      const current = stateStore.read();
      const state = current.browserInteractionMode === "automatic"
        ? current
        : stateStore.update({
            browserInteractionMode: "automatic",
                  zeroRiskProEnabled: false,
          });
      if (state !== current) send("launcher:state-changed", state);
      return { state, credentialsRequired: false, targetMode: "automatic" };
    }
...
async function start() {
  ...
  updateController = createUpdateController({
    ...
    platform: process.platform,
    arch: process.arch,
    packaged: updatesEnabledForAppliance({
      packaged: app.isPackaged,
      devProfile: IS_DEV_PROFILE,
      applianceKind: applianceKind(),
    }),
  ...
  if (IS_DEV_PROFILE) {
    ...
    const persistentRebuild = initialRuntime.config?.runtimeKind === "persistent-rebuild";
    const upgrade = persistentRebuild ? { updated: false } : await runtimeHost.upgradeManagedRuntime();
    if (upgrade.updated) {
      const state = stateStore.update({
        coreSetupComplete: true,
        codexCatalogVerified: false,
    ...
    if (configuredRuntime.configured) {
      ...
      if (saved.zeroRiskProEnabled !== zeroRiskProEnabled) {
        const state = stateStore.update({ zeroRiskProEnabled });
        send("launcher:state-changed", state);
      }
    }
    const runtime = await runtimeSupervisor.startIfConfigured();
    if (runtime.status !== "ready") return runtime;
    const activeRuntime = runtimeHost.runtimeConfigSnapshot();
    if (activeRuntime.config?.runtimeKind === "persistent-rebuild") {
      return { ...runtime, bridgeRouteChanged: false, persistentRebuild: true };
    }
    const route = await runtimeHost.connectBridgeRoute();
    return { ...runtime, bridgeRouteChanged: route.changed === true, persistentRebuild: false };
  })().then(async (runtime) => {
    if (runtime.status === "ready") {
      ...
      if (!runtime.persistentRebuild) startCatalogVerificationMonitor({ logger, stateStore });

...
> **REBUILD NOTE:** この文書は上流の Codex 製品向け資料です。persistent rebuild ブランチでは参照用であり、Goose ChatGPT Web のセットアップ／検証手順ではありません。現行の基準は `AGENTS.md` と `docs/persistent-chat-rebuild-plan.md` です。

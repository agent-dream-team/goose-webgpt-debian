...
> **HERITAGE DOCUMENT:** This DEV harness belongs to the inherited Codex-oriented development workflow. It is not the persistent Goose rebuild launcher, profile, or qualification path. Use `bun run launcher:rebuild` only where the rebuild plan explicitly calls for the source appliance.

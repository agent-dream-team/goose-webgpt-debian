...
This checkout contains the qualified Goose ChatGPT Web V1. Read `docs/persistent-chat-lifecycle.md` before architecture, browser, lifecycle, or recovery work; it owns the non-negotiable persistent-conversation invariants. Then read `docs/persistent-chat-follow-up.md` for the current implementation/qualification gaps. The qualified installed runtime behavior is based on source checkpoint `948010baaf38604d6e877d31245a8afe9bb7ef6a` (`Allow safe pre-acceptance projection rebind`), immutable bundle `1789d88f81716b3f3ac10d3e3c689975dc8ff0545bdcbd135e410dffeff4cd5c`, AppImage SHA-256 `515c291a435c2b8d374fc62b5e071aff8e4a2808e127ca284e5e16e117bbba7a`. Repository HEAD may be newer, but source after that checkpoint is not installed authority until separately packaged and qualified.
...
- **Mandatory browser troubleshooting rule:** once a canonical ChatGPT conversation id exists, treat browser/tab/DOM/Playwright/helper failure as **view failure, not chat failure**. Reopen or refresh that same conversation, wait for the refreshed page to finish hydrating and settle, then classify the server-side turn as `RUNNING`, `STALLED_OR_ERROR`, or `FINAL` using `docs/persistent-chat-lifecycle.md`. `RUNNING` is left alone; `STALLED_OR_ERROR` may receive the standard same-chat GCW continuation only when no unresolved Goose tool work remains; `FINAL` is left terminal for its owner. If the view becomes stale or unreadable again after the observation threshold, refresh the same chat and repeat. The rare pre-canonical first-turn case is the exception: without a durable conversation id, fail closed rather than guessing or resending.
...
- **Inherited upstream surfaces that are not rebuild workflow:** Codex route/catalog integration, Setup/MCP mutation paths, browser smoke → Install models, `Codex Native2`, Zero Risk/manual mode, Codex compaction/subagents, and default `~/.codex-chatgpt-web` / `~/.config/Codex Web GPT` production profile. Obsolete upstream operator documentation has been removed; do not recreate it as current Goose guidance.
- Bigger Context/multipart transport has been removed from this DreamBook product; do not reintroduce it as a provider-side context workaround. Use durable pair recovery and deliberate paired handoff instead.

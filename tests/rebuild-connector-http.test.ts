...
test("conversation-budget exhaustion journals a deterministic failure without executing Goose rendezvous", async () => {
  const f = fixture();
  let rendezvousCalls = 0;
  const rejections: Array<{ turnRef: string; opRef: string; inputHash: string; resultJson: string }> = [];
  const authority = createConnectorOperationAuthority({
    claimOperation: input => ({ kind: "BUDGET_REQUIRED" as const, opRef: input.opRef, seq: 1 }),
    classifyMissingOperationRef: () => ({ kind: "INVALID" as const }),
    markUnreconciled: () => {},
    completeOperation: () => { throw new Error("budget rejection must not complete an executed operation"); },
    rejectOperationForBudget: input => {
      rejections.push(input);
      return { nextOpRef: "op_budget_next" };
    },
  });
  const server = startRebuildConnectorHttpServer({
    port: 0,
    ...
    rendezvous: async () => {
      rendezvousCalls += 1;
      return { outcome: "SUCCESS", dataClass: "task", content: "must-not-execute" };
    },
  });
  servers.push(server);
  const c = client(server, f.authorization);
  try {
    ...
    expect(structured(result)).toMatchObject({
      ok: false,
      output: "REMOTE_CONVERSATION_BUDGET_EXHAUSTED",
      turn_ref: f.turnRef,
      op_ref: f.opRef,
      next_op_ref: "op_budget_next",
    });
    expect(rendezvousCalls).toBe(0);
    expect(rejections).toHaveLength(1);
    const rejected = rejections[0]!;
    expect(rejected.turnRef).toBe(f.turnRef);
    expect(rejected.opRef).toBe(f.opRef);
    expect(rejected.inputHash).toBe(connectorOperationInputHash("tree", { path: "." }));
    expect(rejected.resultJson).toContain("REMOTE_CONVERSATION_BUDGET_EXHAUSTED");
  } finally {
    await c.client.close().catch(() => {});
    f.broker.close();
  }

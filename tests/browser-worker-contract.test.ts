...
function thinkSlashFixture() {
  ...
  const noEffortControls = { filter: () => noEffortControls, count: async () => 0 };
  const composerForm = {
    getByRole: () => ({ filter: () => controls }),
    locator: (selector: string) => selector === CHATGPT_EFFORT_CONTROL_SELECTOR ? noEffortControls
      : selector === CHATGPT_COMPOSER_SELECTOR ? composer
      : composer,
    page: () => page,
  };
...
test("Full mode fails closed when ChatGPT exposes completion without a post-tool final answer", () => {
  ...
  expect(() => tracker.update(partialLookingFinal, 2_000))
    .toThrow("completed without producing a final answer after its last tool call");
});

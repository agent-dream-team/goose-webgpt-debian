...
test("connector selection triggers the configured Goose Native mention instead of the retired Codex-only @c trigger", async () => {
  ...
  const selectedConnector = {
    waitFor: async () => { calls.push(["waitForSelectedConnector"]); },
    count: async () => 1,
  };
  const selectedComposer = {
...
test("connector selection fails closed when the mention menu never exposes exactly one exact-name row", async () => {
  const appResult = {
    waitFor: async () => {},
    count: async () => 0,
  };
  const menuRows = {
    ...
    allInnerTexts: async () => ["Some Other Connector\nextra"],
  };
  const initialComposer = {
    fill: async () => {},
    focus: async () => {},
...
test("effort selection uses structural menu indices instead of localized labels", () => {
  ...
  expect(workerSource).toContain("CHATGPT_EFFORT_POWER_CONTROL_SELECTOR");
  expect(workerSource).toContain("CHATGPT_EFFORT_POWER_SLIDER_SELECTOR");
  ...
  expect(sessionSource).toContain('[aria-keyshortcuts~="ArrowLeft"]');
  expect(sessionSource).toContain('[aria-keyshortcuts~="ArrowRight"]:has([role="slider"])');
  expect(sessionSource).toContain('[role="slider"][aria-valuemin][aria-valuemax][aria-valuenow]');
  ...
  expect(workerSource).toContain('page.keyboard.press(selected < mode.uiEffortIndex ? "ArrowRight" : "ArrowLeft")');
